...
const ResultSummary: React.FC<ResultSummaryProps> = ({ session, onRestart, onReviewMode }) => {
  const [aiFeedback, setAiFeedback] = useState<string>('正在为您准备老师评语...');
  ...
  useEffect(() => {
    const fetchFeedback = async () => {
      const fb = await getAIFeedback(session.score, session.problems.length, duration);
      setAiFeedback(fb);
    };
    fetchFeedback();
  }, [session.score, session.problems.length, duration]);
  ...
  return (
    <div className="space-y-6 animate-in zoom-in-95 duration-500">
      ...
      <div className="bg-indigo-600 text-white p-5 rounded-3xl relative overflow-hidden shadow-lg">
        <h4 className="font-black text-xs mb-2 flex items-center gap-2 text-indigo-100 uppercase tracking-widest">
          <span>AI 导师点睛</span>
        </h4>
        <p className="leading-relaxed italic text-sm font-medium">
          "{aiFeedback}"
        </p>
      </div>
